...
<?php 
	if (isset($_GET['edit'])) {
		...
		$update = true;
		$record = mysqli_query($db, "SELECT * FROM category WHERE category_id=$categoryID");
...
<?php $results = mysqli_query($db, "SELECT * FROM category"); ?>
			...
			<tbody>
				...
				<?php while ($row = mysqli_fetch_array($results)) { ?>
					<tr>
						...
						<td>
							<a href="category_front.php?edit=<?php echo $row['category_id']; ?>" class="edit_btn" >Edit</a>
						</td>
						<td>
							<a href="category_back.php?del=<?php echo $row['category_id']; ?>" class="dlt_btn">Delete</a>
						</td>
					</tr>
				<?php } ?>
			</tbody>
	...
	<div>
		...
		<form method="post" action="category_back.php" style="background: #fffdf2;">
			<div>
				<?php if ($update == true): ?>
					<h3>Update Data</h3>
				<?php else: ?>
			...
			<div class="input-group">
				...
				<input type="number" name="category_id" value="<?php echo $categoryID; ?>">
			</div>
