<?php  include('film_text_back.php'); ?>
<?php
...
<?php 
	if (isset($_GET['edit'])) {
		$filmID = $_GET['edit'];
		$update = true;
		$record = mysqli_query($db, "SELECT * FROM film_text WHERE film_id=$filmID");
...
<?php $results = mysqli_query($db, "SELECT * FROM film_text"); ?>
			...
			<tbody>
				...
				<?php while ($row = mysqli_fetch_array($results)) { ?>
					<tr>
						...
						<td>
							<a href="film_text_front.php?edit=<?php echo $row['film_id']; ?>" class="edit_btn" >Edit</a>
						</td>
						<td>
							<a href="film_text_back.php?del=<?php echo $row['film_id']; ?>" class="dlt_btn">Delete</a>
						</td>
					</tr>
				<?php } ?>
			</tbody>
	...
	<div>
		...
		<form method="post" action="film_text_back.php" style="background: #e6e6e6;">
			<div>
				<?php if ($update == true): ?>
					<h3>Update Data</h3>
				<?php else: ?>
			...
			<div class="input-group">
				...
				<input type="text" name="description" value="<?php echo $title; ?>">
			</div>
