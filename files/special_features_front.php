<?php  include('special_features_back.php'); ?>
<?php
...
<?php 
	if (isset($_GET['edit'])) {
		$featureID = $_GET['edit'];
		$update = true;
		$record = mysqli_query($db, "SELECT * FROM special_features WHERE feature_id=$featureID");
		
		if (count($record) == 1){
			$n = mysqli_fetch_array($record);
			$featureID = $n['feature_id'];
			$sf = $n['special_feature'];
		}
	}
?>
...
<?php $results = mysqli_query($db, "SELECT * FROM special_features"); ?>
			...
			<tbody>
				<?php while ($row = mysqli_fetch_array($results)) { ?>
					<tr>
						...
						<td>
							<a href="special_features_front.php?edit=<?php echo $row['feature_id']; ?>" class="edit_btn" >Edit</a>
						</td>
						<td>
							<a href="special_features_back.php?del=<?php echo $row['feature_id']; ?>" class="dlt_btn">Delete</a>
						</td>
					</tr>
				<?php } ?>
			</tbody>
	...
	<div>
		...
		<form method="post" action="special_features_back.php" style="background: #f2f7ff;">
			<div>
				<?php if ($update == true): ?>
					<h3>Update Data</h3>
				<?php else: ?>
			...
			<div class="input-group">
				...
				<input type="number" name="feature_id" value="<?php echo $featureID; ?>">
			</div>

			<div class="input-group">
				<label>Special Feature</label>
				<input type="text" name="special_feature" value="<?php echo $sf; ?>" required>
			</div>
