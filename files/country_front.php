...
<?php 
	if (isset($_GET['edit'])) {
		$countryID = $_GET['edit'];
		$update = true;
		$record = mysqli_query($db, "SELECT * FROM country WHERE country_id=$countryID");
...
<?php $results = mysqli_query($db, "SELECT * FROM country"); ?>
			...
			<tbody>
				...
				<?php while ($row = mysqli_fetch_array($results)) { ?>
					<tr>
						...
						<td>
							<a href="country_front.php?edit=<?php echo $row['country_id']; ?>" class="edit_btn" >Edit</a>
						</td>
						<td>
							<a href="country_back.php?del=<?php echo $row['country_id']; ?>" class="dlt_btn">Delete</a>
						</td>
					</tr>
				<?php } ?>
			</tbody>
	...
	<div>
		...
		<form method="post" action="country_back.php" style="background: #f2f7ff;">
			<div>
				<?php if ($update == true): ?>
					<h3>Update Data</h3>
				<?php else: ?>
			...
			<div class="input-group">
				...
				<input type="number" name="country_id" value="<?php echo $countryID; ?>">
			</div>
