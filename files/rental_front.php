...
<?php 
	if (isset($_GET['edit'])) {
		$rentalID = $_GET['edit'];
		$update = true;
		$record = mysqli_query($db, "SELECT * FROM rental WHERE rental_id=$rentalID");
		
		if (count($record) == 1){
			$n = mysqli_fetch_array($record);
			$rentalID = $n['rental_id'];
			$rentDate = $n['rental_date'];
...
<?php $results = mysqli_query($db, "SELECT * FROM rental"); ?>
			...
			<tbody>
				...
				<?php while ($row = mysqli_fetch_array($results)) { ?>
					<tr>
						...
						<td>
							<a href="rental_front.php?edit=<?php echo $row['rental_id']; ?>" class="edit_btn" >Edit</a>
						</td>
						<td>
							<a href="rental_back.php?del=<?php echo $row['rental_id']; ?>" class="dlt_btn">Delete</a>
						</td>
					</tr>
				<?php } ?>
			</tbody>
	...
	<div>
		...
		<form method="post" action="rental_back.php" style="background: #fff2f2;">
			<div>
				<?php if ($update == true): ?>
					<h3>Update Data</h3>
				<?php else: ?>
			...
			<div class="input-group">
				...
				<input type="number" name="rental_id" value="<?php echo $rentalID; ?>">
			</div>

			<div class="input-group">
				<label>Rental Date</label>
				<input type="datetime-local" name="rental_date" value="<?php echo $rentDate; ?>">
			</div>

			<div class="input-group">
				<label>Inventory ID</label>
				<input type="number" name="inventory_id" value="<?php echo $inventoryID; ?>" required>
			</div>

			<div class="input-group">
				<label>Customer ID</label>
				<input type="number" name="customer_id" value="<?php echo $customerID; ?>" required>
			</div>

			<div class="input-group">
				<label>Return Date</label>
				<input type="datetime-local" name="return_date" value="<?php echo $return_date; ?>">
			</div>

			<div class="input-group">
				<label>Staff ID</label>
				<input type="number" name="staff_id" value="<?php echo $staffID; ?>" required>
			</div>
