<?php include('film_actor_back.php'); ?>
<?php
...
<?php 
	if (isset($_GET['edit'])) {
		// $actorID = $_GET['edit'];
		// $filmID = $_GET['edit'];
		$filmActorID = $_GET['edit'];
		$update = true;
		// $record = mysqli_query($db, "SELECT * FROM film_actor WHERE actor_id=$actorID AND film_id=$filmID");
		$record = mysqli_query($db, "SELECT * FROM film_actor WHERE film_actor_id=$filmActorID");
...
<?php $results = mysqli_query($db, "SELECT * FROM film_actor"); ?>
			...
			<tbody>
				...
				<?php while ($row = mysqli_fetch_array($results)) { ?>
					<tr>
						...
						<td>
							<a href="film_actor_front.php?edit=<?php echo $row['film_actor_id'];?>" class="edit_btn" >Edit</a>
						</td>
						<td>
							<a href="film_actor_back.php?del=<?php echo $row['film_actor_id'];?>" class="dlt_btn">Delete</a>
						</td>
					</tr>
				<?php } ?>
			</tbody>
	...
	<div>
		...
		<form method="post" action="film_actor_back.php" style="background: #fff2f9;">
			<div>
				<?php if ($update == true): ?>
					<h3>Update Data</h3>
				<?php else: ?>
