...
<?php 
	if (isset($_GET['edit'])) {
		$cityID = $_GET['edit'];
		$update = true;
		$record = mysqli_query($db, "SELECT * FROM city WHERE city_id=$cityID");
...
<?php $results = mysqli_query($db, "SELECT * FROM city"); ?>
			...
			<tbody>
				...
				<?php while ($row = mysqli_fetch_array($results)) { ?>
					<tr>
						...
						<td>
							<a href="city_front.php?edit=<?php echo $row['city_id']; ?>" class="edit_btn" >Edit</a>
						</td>
						<td>
							<a href="city_back.php?del=<?php echo $row['city_id']; ?>" class="dlt_btn">Delete</a>
						</td>
					</tr>
				<?php } ?>
			</tbody>
	...
	<div>
		...
		<form method="post" action="city_back.php" style="background: #f2fdff;">
			<div>
				<?php if ($update == true): ?>
					<h3>Update Data</h3>
				<?php else: ?>
			...
			<div class="input-group">
				<label>City ID</label>
				<input type="number" name="city_id" value="<?php echo $cityID; ?>">
			</div>
			...
			<div class="input-group">
				...
				<input type="number" name="country_id" value="<?php echo $countryID; ?>" required>
			</div>
