...
<?php 
	if (isset($_GET['edit'])) {
		$languageID = $_GET['edit'];
		$update = true;
		$record = mysqli_query($db, "SELECT * FROM language WHERE language_id=$languageID");
...
<?php $results = mysqli_query($db, "SELECT * FROM language"); ?>
			...
			<tbody>
				...
				<?php while ($row = mysqli_fetch_array($results)) { ?>
					<tr>
						...
						<td>
							<a href="language_front.php?edit=<?php echo $row['language_id']; ?>" class="edit_btn" >Edit</a>
						</td>
						<td>
							<a href="language_back.php?del=<?php echo $row['language_id']; ?>" class="dlt_btn">Delete</a>
						</td>
					</tr>
				<?php } ?>
			</tbody>
	...
	<div>
		...
		<form method="post" action="language_back.php" style="background: #fff2f2;">
			<div>
				<?php if ($update == true): ?>
					<h3>Update Data</h3>
				<?php else: ?>
			...
			<div class="input-group">
				...
				<input type="number" name="language_id" value="<?php echo $languageID; ?>">
			</div>
